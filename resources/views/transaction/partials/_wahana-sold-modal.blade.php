{{--
    Partial: Wahana Sold Modal
    Includes: modal HTML + AJAX JS + scrollbar style
    Usage: @include('transaction.partials._wahana-sold-modal')
    Requires: route('transaction.wahana-sold') available
--}}

<!-- Wahana Sold Modal -->
<div class="modal fade" id="modalWahanaSold" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body bg-light p-3">
                <!-- Loading -->
                <div id="wsLoading" class="text-center py-5">
                    <div class="spinner-border text-primary" role="status"></div>
                    <p class="mt-2 text-muted">Memuat data wahana...</p>
                </div>

                <!-- Content -->
                <div id="wsContent" style="display:none;">
                    <p class="text-muted small text-uppercase fw-bold mb-2">
                        <i class="ti ti-trophy text-warning me-1"></i>Top 3 Terjual
                    </p>
                    <div class="row g-3 mb-4" id="wsTop3"></div>

                    <div class="card border-0 shadow-sm">
                        <div
                            class="card-header bg-white d-flex align-items-center justify-content-between border-bottom">
                            <h6 class="mb-0 fw-bold"><i class="ti ti-list me-2 text-primary"></i>Semua Wahana Terjual
                            </h6>
                            <span class="badge bg-primary" id="wsTotalBadge">0 jenis</span>
                        </div>
...
<script>
    function openWahanaSoldModal() {
        const modal = new bootstrap.Modal(document.getElementById('modalWahanaSold'));
        modal.show();

        const loading = document.getElementById('wsLoading');
        const content = document.getElementById('wsContent');
        const top3El = document.getElementById('wsTop3');
        const allListEl = document.getElementById('wsAllList');
        const badge = document.getElementById('wsTotalBadge');
        ...
        fetch('{{ route('transaction.wahana-sold') }}', {
                headers: {
            ...
            .then(data => {
                ...
                if (data.top3 && data.top3.length > 0) {
                    ...
                    top3El.innerHTML =
                        '<div class="col-12 text-center text-muted py-3">Belum ada wahana terjual.</div>';
                }
