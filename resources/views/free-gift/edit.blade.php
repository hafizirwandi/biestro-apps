...
@section('content')
    ...
    <form class="row g-3" method="post" action="{{ route('free-gift.update', $data->id) }}">
        @csrf
        @csrf
        @method('PUT')
        <div class="row">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-12 col-md-12 mb-3">
                                <label class="form-label">Is Multiple</label>
                                <select name="is_multiple" class="form-select" placeholder="Enter Text" required>
                                    <option value="1" @selected($data->is_multiple == 1)>Multiple</option>
                                    <option value="0" @selected($data->is_multiple == 0)>Not Multiple</option>
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-6 col-md-6 mb-3">
                                ...
                                <input type="text" name="min_purchase" value="{{ $data->min_purchase }}"
                                    class="form-control auto-numeric" placeholder="Enter Text" required />
                            </div>
                            <div class="col-6 col-md-6 mb-3">
                                ...
                                <select name="is_active" class="form-select" placeholder="Enter Text" required>
                                    ...
                                    <option value="0" @selected($data->is_active == 0)>Not Active</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-12 col-md-12 mb-3">
            ...
            <div class="col-md-6">
                ...
                <div id="listWahana" style="padding: 0">

                    {{-- @foreach ($data->wahanas as $d)
                        <div class="d-flex gap-2 p-0 mb-3 align-items-center list-wahana">
                            <select name="wahana_id[]" class="form-select">
                                @foreach ($wahana as $r)
                                    <option value="{{ $r->id }}">{{ $r->name }}</option>
                                @endforeach
                            </select>
                            <input type="text" name="qty[]" style="width: 70px" value="{{ $d->pivot->qty }}"
                                class="form-control">
                            <a href="javascript:;" class="btn btn-danger btn-remove-wahana"><i
...
@section('script')
    ...
    <script>
        ...
        const selectedWahanas = @json($selectedWahanas ?? []);

        function renderWahanaRowsFromEdit() {
            if (!Array.isArray(selectedWahanas)) return;

            selectedWahanas.forEach(item => {
                const row = createWahanaRow();
                const select = row.querySelector('select[name="wahana_id[]"]');
                const inputQty = row.querySelector('input[name="qty[]"]');

                select.value = item.wahana_id;
