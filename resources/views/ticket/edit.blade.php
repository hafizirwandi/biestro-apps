  ...
  <div class="text-center mb-4">
      <h3 class="mb-2 modal-title">Edit Ticket</h3>
  </div>
  <form class="row g-3" method="post" action="{{ route('ticket.update', $data->id) }}">
      @csrf
      @method('PUT')
      <div class="col-12 col-md-12">
          ...
          <select name="wahana_id" class="form-select" placeholder="Enter Text" required>
              ...
              @foreach ($wahana as $r)
                  <option value="{{ $r->id }}" @selected($data->wahana_id)>
                      {{ $r->name }}</option>
              @endforeach
          </select>
      </div>
      ...
      <div class="col-12 col-md-12">
          ...
          <input type="text" name="price" class="form-control auto-numeric" value="{{ $data->price }}"
